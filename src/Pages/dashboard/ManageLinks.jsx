...
export default function ManageLinks() {
  ...
  const API_URL = import.meta.env.VITE_API_URL; // <-- Add this to .env
  ...
  const resetForm = () => {
    setFormData({ title: { en: "", tl: "" }, url: "", description: { en: "", tl: "" }, icon: "" });
    setEditingId(null);
  };
  ...
  const startEditing = (link) => {
    ...
    setFormData({
      title: link.title || { en: "", tl: "" },
      url: link.url || "",
      description: link.description || { en: "", tl: "" },
      icon: link.icon || "",
    });
    setShowModal(true);
  };
  ...
  async function handleDelete(id) {
    ...
    try {
      const res = await fetch(`${API_URL}/external-links/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        ...
        setMessage(t("errorDeletingLink"));
      }
    } catch (error) {
      console.error(error);
      setMessage(t("errorDeletingLink"));
        ...
        {links.map((link) => (
          <div key={link.id} className="card">
            ...
            <div className="flex space-x-2 mt-4">
              <button onClick={() => startEditing(link)} className="secondary-btn flex items-center space-x-1 flex-1">
                <Edit className="w-4 h-4" />
                <span>{t("edit")}</span>
              </button>
              <button onClick={() => handleDelete(link.id)} className="danger-btn flex items-center space-x-1">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {editingId ? t("editLink") : t("addLink")}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t("titleTagalog")}</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t("descriptionTagalog")}</label>
                  <textarea
                    value={formData.description.tl}
                    onChange={(e) => setFormData({ ...formData, description: { ...formData.description, tl: e.target.value } })}
                    rows="3"
                    className="input-field"
                  />
                </div>
