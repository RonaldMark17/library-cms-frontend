...
export default function ManageAnnouncements() {
  ...
  const resetForm = () => {
    setFormData({ title: "", content: "", priority: "medium", published_at: "", expires_at: "", image: null });
    setEditingId(null);
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchAnnouncements();
      } else setMessage(t("errorDeletingAnnouncement"));
    } catch {
  ...
  if (loading) {
    return <div className="flex justify-center items-center min-h-[400px]"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600"></div></div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { resetForm(); setShowModal(true); }} className="primary-btn flex items-center space-x-2">
          <Plus className="w-5 h-5" /><span>{t("addAnnouncement")}</span>
        </button>
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea value={formData.content} onChange={e => setFormData({...formData, content:e.target.value})} rows="6" className="input-field" required/>
              </div>
